...
const getModelsByMake = async function (array) {
    let arrayOneLength = array.length;

    const makers = [];

    for (let row = 0; row < arrayOneLength; row++) {
        makers.push(array[row][0]);
    }

    async function GET(make) {
    ...
    for (let i = 0; i < makers.length; i++) {
        const make = makers[i];
        await GET(make).then(data => {
            const resultArray = data.Results;
            const obj = {};
            obj["brandName"] = make;
            obj["models"] = [];

            resultArray.forEach(modelObj => {
                obj.models.push(modelObj["Model_Name"]);
            });
...
cmm.forEach((row, rowIndex, matrix) => {
    const make = row[0];
    const makeId = rowIndex + 1;
    const insertRow = `insert into makers (id, name) values (${makeId}, '${make}');`;
    console.log(insertRow);
    row.slice(2).forEach((model, index) => {

        const insertRow = `insert into models (id, name, maker_id) values (${++modelId}, '${model}', ${makeId});`;
        console.log(insertRow);
    })
});