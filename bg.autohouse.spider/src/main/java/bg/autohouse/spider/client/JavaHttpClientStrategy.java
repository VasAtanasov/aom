...
public class JavaHttpClientStrategy implements HttpStrategy
{
    ...
    public <T> Response<T> call(final RequestMetadata metadata, ResponseBodyHandler<T> handler)
    {
        try
        {
            return callAsync(metadata, handler).get();
        }
    ...
    @Override
    public <T> CompletableFuture<Response<T>> callAsync(RequestMetadata metadata, ResponseBodyHandler<T> handler)
    {
        ...
        switch (metadata.method())
        {
            ...
            default -> throw new UnsupportedOperationException("Unsupported method: " + metadata.method());
        }
    ...
    private <T> Function<HttpResponse<InputStream>, Response<T>> toResponse(ResponseBodyHandler<T> handler)
    {
        return (response) -> {
