...
/**
 * An example command-line application that runs through the web-based OAuth flow (using {@link
 * DbxWebAuth}).
 */
public class Main {
   ...
   * since it allows for more efficient processing of the file contents on the server side and also
   * allows partial uploads to be retried (e.g. network connection problem will not cause you to
    ...
    for (int i = 0; i < CHUNKED_UPLOAD_MAX_ATTEMPTS; ++i) {
      ...
      try (InputStream in = new FileInputStream(localFile)) {
        ...
        CommitInfo commitInfo =
            CommitInfo.newBuilder(dropboxPath)
                .withMode(WriteMode.ADD)
                .withClientModified(new Date(localFile.lastModified()))
                .build();
        FileMetadata metadata =
        ...
        if (ex.errorValue.isLookupFailed()
          ...
          // the expected offset according to the server and try again.
          uploaded =
              ex.errorValue.getLookupFailedValue().getIncorrectOffsetValue().getCorrectOffset();
          continue;
        } else {
          // some other error occurred, give up.
          System.err.println("Error uploading to Dropbox: " + ex.getMessage());
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (args.length != 3) {
      ...
      System.out.println("");
      System.out.println(
          " <local-path>: The path to a local file whose contents you want to upload.");
      System.out.println("");
