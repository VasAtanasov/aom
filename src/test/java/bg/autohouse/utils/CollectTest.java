...
import static bg.autohouse.util.Collect.greatestAfterGroupingBy;
import static bg.autohouse.util.Collect.groupingByIdOf;
import static bg.autohouse.util.Collect.idList;
import static bg.autohouse.util.Collect.idSet;
import static bg.autohouse.util.Collect.indexingBy;
import static bg.autohouse.util.Collect.indexingByIdOf;
import static bg.autohouse.util.Collect.leastAfterGroupingBy;
import static bg.autohouse.util.Collect.mappingAndCollectingFirst;
import static bg.autohouse.util.Collect.mappingTo;
import static bg.autohouse.util.Collect.nullSafeGroupingBy;
import static bg.autohouse.util.Collect.nullSafeGroupingByIdOf;
...
public class CollectTest {
  ...
  public void testIdList() {
    ...
    assertEquals(
        Ints.asList(b1.id, b2.id, b1.id), Stream.of(a1, a2, a3, a4).map(A::getB).collect(idList()));
  }
  ...
  public void testIdSet() {
    ...
    assertEquals(
        Sets.newHashSet(b1.id, b2.id), Stream.of(a1, a2, a3, a4).map(A::getB).collect(idSet()));
  }
  ...
  public void testIndexingBy() {
    ...
    assertEquals(
        ImmutableMap.of(b1, a1, b2, a2, b3, a3),
        Stream.of(a1, a2, a3).collect(indexingBy(A::getB)));
  }
  ...
  public void testMappingTo() {
    ...
    assertEquals(
        ImmutableMap.of(a1, b1, a2, b2, a3, b1), Stream.of(a1, a2, a3).collect(mappingTo(A::getB)));
  }
  ...
  private static class A implements Identifiable<Integer> {
    final int id;
    final B b;
  ...
  private static class B implements Identifiable<Integer> {
    final int id;
